...
public class Crime
{
    ...
    public static String EachState(CrimeType crimeType, TimeRange time, WeekType weekType, String location) throws IOException {
        ...
        Log.d("EachState: ", "GetCount: " + (double)((double)CriminalLoader.GetCount(crimeType, time) /  CriminalLoader.Avg().get(crimeType)));
        Log.d("EachState: ", "RegionManager: " + RegionManager.GetValue(location));
        Log.d("EachState: ", "Avg(): " +  Average.get(crimeType,weekType));
        Log.d("EachState: ", "avgCount: " + avgCount);
        // 시간대에 따른 범죄 발생을 확인하여 범죄 상태를 반환
        return calculateCrimeState(avgCount * Average.get(crimeType,weekType));
    }
    ...
    private static String  calculateCrimeState(double avgCount) {
        if (avgCount <= 0.4)
            return "매우좋음";
        else if (avgCount <= 0.7)
            return "좋음";
        else if (avgCount <= 1.0)
            return "보통";
